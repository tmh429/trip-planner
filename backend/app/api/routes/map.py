...
async def plan_route(
    origin_address: str = Query(..., description="起点地址"),
    destination_address: str = Query(..., description="终点地址"),
    origin_city: Optional[str] = Query(None, description="起点城市"),
    destination_city: Optional[str] = Query(None, description="终点城市"),
    route_type: str = Query("walking", description="路线类型: walking/driving/transit"),
):
    """
    规划路线
    """
...
async def health_check():
    ...
    try:
        ...
        return {
            "status": "healthy",
            "service": "map-service",
            "mcp_tools_count": len(service._amap_mcp_tool._available_tools)
        }
    except Exception as e:
        raise HTTPException(
            status_code=503,
